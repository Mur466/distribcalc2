...
func (t *Task) SaveTask() {
	...
	if t.Task_id < 0 {
		...
		err := db.Conn.QueryRow(context.Background(), `
			INSERT INTO tasks (data, user_id) VALUES ($1, $2) returning task_id;
			`, data, t.User_id).Scan(&t.Task_id)
		if err != nil {
...
func InitTasks() {
	...
	for rows.Next() {
		task := Task{}
		task_data := []byte{}
		err := rows.Scan(&task.Task_id, &task_data, &task.User_id)
		if err != nil {
...
func ListTasks(user_id int, limit int, offset int) []*Task {
	strlimit := "ALL"
	if limit > 0 {
