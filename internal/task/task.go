...
func NewTask(expr string, ext_id string, user_id int) *Task {
	double := CheckUnique(ext_id, user_id)
	if double != nil {
		return double
	}

	t := &Task{Task_id: -1, Expr: expr, Ext_id: ext_id, User_id: user_id, TreeSlice: make([]*Node, 0), DateCreated: time.Now()}
...
func CheckUnique(Ext_id string, user_id int) *Task {
	if Ext_id == "" {
	...
	for _, t := range Tasks {
		if t.Ext_id == Ext_id && t.User_id == user_id {
			return t
		}
	}
	return nil
